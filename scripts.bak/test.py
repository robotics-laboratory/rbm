...
class HWStatusTest(BaseTest):
    ...
    async def test(self):
        ...
        assert not fail, f"HW sensors failed: {', '.join(failed)}"
...
class WheelTest(BaseTest):
    ...
    async def test(self):
        try:
            for name, (target_idx, speed_idx) in self.WHEEL.items():
                targets = [0.0, 0.0, 0.0, 0.0]
                targets[target_idx] = self.TEST_SPEED

                data = ", ".join(str(value) for value in targets)

                await self.docker_shell(
                    "ros2 topic pub --once "
                    "/hardware/wheel_targets std_msgs/msg/Float32MultiArray "
                    f"\"{{data: [{data}]}}\"",
                    timeout=10,
                )
...
class MoveTest(BaseTest):
    ...
    async def send_cmd(self, linear, angular, sec):
        hz = 10
        count = int(sec * hz)

        await self.shell(
            "docker exec -e RMW_IMPLEMENTATION=rmw_fastrtps_cpp "
            "ros /ros_entrypoint.sh ros2 topic pub "
            f"-r {hz} "
            f"--times {count} "
    ...
    async def test(self):
        try:
            self.log("Forward", color="yellow")
            await self.send_cmd(0.15, 0.0, 2.0)

            self.log("Turn", color="yellow")
            await self.send_cmd(0.0, -1.2, 2.1)
